...
```{r gender salary, fig.cap="Salary Distribution for Men and Women in Each Quarter"}
...
```{r fig1, fig.cap="salary difference across leadership, seniority, and gender for level, fixing productivity"}
ldr_slry <- current %>% 
  filter(financial_q == '2020 Q1') %>% 
  group_by(employee_id) %>% 
  ggplot(aes(x = productivity, y = salary, color = as.factor(leadership_for_level)))+
  geom_point()+
  labs(color = "Leadership Score")
```
